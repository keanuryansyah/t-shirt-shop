    ...
    // header end js
     ...
     document.querySelector(".filter").addEventListener("click", function() {
        ...
        // show filter and hide filter
        document.querySelector(".isiFilter").classList.toggle("active");
    })
                    ...
                    addToCartTrigger.addEventListener("click", function() {
                        ...
                        popadd.onreadystatechange = function() {
                            if( popadd.readyState == 4 && popadd.status == 200 ) {
                                ...
                                viewCartAdded.addEventListener("click", function() {
    ...
    let triggerKategori = document.querySelectorAll(".kategoriShopCtn button");

    triggerKategori.forEach(function(tk) {

        // trigger kategori di klik, kirimkan isi text trigger yang di klik
        tk.addEventListener("click", function() {

        if( tk.classList.contains("active") ) {
            return false;
        }

        // netralin all tk
        for( let allTk of triggerKategori ) {
           allTk.classList.remove("active");
        }
        ...
        const tkVal = tk.innerText;

        let xhr = new XMLHttpRequest();
            
        xhr.onreadystatechange = function() {
            if( xhr.readyState == 4 && xhr.status == 200 ) {
                // ambil secton best sell sebelumnya
                const changeBestSell = document.getElementById("sectionBestSell");

                // ganti section best sell sebelum nya dengan hasil ajax
                changeBestSell.innerHTML = xhr.responseText;
    ...
    triggerView.forEach(function(trigView) {
        ...
        xhr.open("POST", "ajaxkategori.php?nameprod=" + tkVal, true);
        xhr.send();
